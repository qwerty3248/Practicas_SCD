...
const int
   ...
   etiq_consumidor = 1,
   elemento_borrado = 27,
   id_buffer             = num_procesos_esperado-1 ,
   tam_vector            = 8;
...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      if ( num_celdas_ocupadas == 0 )               // si buffer vacío
          flag = etiq_productor;
          if (limpieza){
          	cout<<"Se acabo la limpieza\n";
          	limpieza = false;
          }
      else if ( num_celdas_ocupadas == tam_vector || limpieza) // si buffer lleno
      ...
      switch( tag ) // leer emisor del mensaje en metadatos
      {
         case etiq_productor: // si ha sido el productor: insertar en buffer
            ...
            if (valor == elemento_borrado){
            	limpieza = true;
            	cout<<"Comienzo de la limpieza \n";
            }
            break;
...
int main( int argc, char *argv[] )
{
   ...
   if ( num_procesos_esperado == num_procesos_actual )
   {
      ...
      if ( id_propio >= 0 && id_propio < NC)
         funcion_consumidor(id_propio);
      else if ( id_propio ==  id_buffer)
         funcion_buffer();
      else
         funcion_productor(id_propio - NC);//el 1 es por el buffer si hubiera 2 buffers habria que restar poner un 2
   }
   else
   {
